...
function parse(html: string): Article[] {
  ...
  const links = $('a')
    .toArray()
    .filter((elem) => elem.attribs['data-post-id'])
    .map((elem) => elem.attribs.href);
  const dates = $('time')
    .toArray()
    .map((elem) => elem.attribs.datetime);
  const articleElement = $('.col.u-xs-size12of12')
    .toArray()
